...
public class WitConnector : MonoBehaviour
{
    ...
    private int instructionTimes = 5;

    [SerializeField]
    private SanctuaryPet pet;
...
#region GazeFocus
    bool HasFocus()
    {
        ...
        if (angle < ozFov || distance < distanceThreshold)
        {
            focusCount++;
        }
        else
...
#region Wit
    ...
    void StartListening() {
        ...
        pet.Listening(true);
        if (instructionCount < instructionTimes)
        {
            pet.DisplayThought();
            instructionCount++;
        }
    ...
    // when the game resets, ensure the thought bubble works again
