...
public class LightBeam : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            Vector3 lookAt = (_beamBase.position - SanctuaryExperience.Instance._mainCamera.transform.position).normalized;
            if (Vector3.Dot(lookAt, SanctuaryExperience.Instance._mainCamera.transform.forward) >= _viewCone)
            {
                _observed = true;
                _beamParticles.Play();
    ...
    public void Prepare(Vector3 toyPos)
    {
        ...
        transform.position = new Vector3(toyPos.x, SanctuaryExperience.Instance.GetFloorHeight(), toyPos.z);
    ...
    {
        // TODO: something broke audio when the project upgraded to v38
        //beamLoop.Stop();
        //beamOutro.Play(beamBase.position); // need to use position so SetActive(false) doesn't kill the AudioSource
