...
public class OVRPassthroughLayer : MonoBehaviour
{
	...
	#region Experimental Interface
	// If true, this layer is composited according to the blend factors specified in `srcBlendFactor and `dstBlendFactor`.
	public bool overrideBlendFactors = false;

	// A factor by which the color value of the layer is multiplied during compositing. This field ignored if `overrideBlendFactors` is false.
	public OVRPlugin.BlendFactor srcBlendFactor = OVRPlugin.BlendFactor.SrcAlpha;

	// A factor by which the previous color value is multiplied during compositing. This field ignored if `overrideBlendFactors` is false.
	public OVRPlugin.BlendFactor dstBlendFactor = OVRPlugin.BlendFactor.OneMinusSrcAlpha;

	// Internal color map type used for keyboard hands presence.
	// DO NOT USE. This will be removed once keyboard hand presence is implemented as a separate feature of the Passthrough API.
	public void SetColorMapHandsContrast(float offset, float multiplier, float target)
	{
		colorMapType = ColorMapType.HandsContrast;
		colorMapEditorType = ColorMapEditorType.Custom;
		AllocateColorMapData();

		byte[] bytes = BitConverter.GetBytes(offset);
		Buffer.BlockCopy(bytes, 0, colorMapData, 0, 4);
		bytes = BitConverter.GetBytes(multiplier);
		Buffer.BlockCopy(bytes, 0, colorMapData, 4, 4);
		bytes = BitConverter.GetBytes(target);
		Buffer.BlockCopy(bytes, 0, colorMapData, 8, 4);
...
#if OVR_INTERNAL_CODE
		passthroughOverlay.overrideBlendFactors = overrideBlendFactors;
		passthroughOverlay.srcBlendFactor = srcBlendFactor;
		passthroughOverlay.dstBlendFactor = dstBlendFactor;
#endif
				...
				else
				{
					...
					switch (style.TextureColorMapType)
					{
						...
						case ColorMapType.MonoToMono:
							...
							break;
#if OVR_INTERNAL_CODE
						case ColorMapType.HandsContrast:
							style.TextureColorMapDataSize = 3 * 4; // 3 floats
							break;
#endif
						default:
