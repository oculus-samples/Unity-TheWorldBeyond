...
#region Interface
	...
	// If true, this overlay is composited according to the blend factors specified in `srcBlendFactor and `dstBlendFactor`.
	public bool overrideBlendFactors = false;

	// A factor by which the color value of the overlay is multiplied during compositing. This field ignored if `overrideBlendFactors` is false.
	...
	[Tooltip("When checked, the layer will use sharpen")]
...
#if OVR_INTERNAL_CODE
			hidden,	overrideBlendFactors, srcBlendFactor, dstBlendFactor, useEfficientSupersample, useSharpen);
#else
			hidden);
#endif
