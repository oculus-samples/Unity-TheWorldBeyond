...
public class OVRManager : MonoBehaviour, OVRMixedRealityCaptureConfiguration
{
  ...
  /// @params (UInt64 requestId, bool result, UInt64[] spaces, SpatialEntityUuid[] uuids, UInt32 numSpaces, Principal[] principals, UInt32 numPrincipals)
  /// </summary>
  public static event Action<UInt64, bool, UInt64[], OVRPlugin.SpatialEntityUuid[], UInt32, OVRPlugin.Principal[], UInt32> SpatialEntityShareSpace;
		...
		while(OVRPlugin.PollEvent(ref eventDataBuffer))
		{
			switch(eventDataBuffer.EventType)
			{
				...
				case OVRPlugin.EventType.SpatialEntityStorageEraseResult:
					...
					break;
#if OVR_INTERNAL_CODE
				case OVRPlugin.EventType.SpatialEntityShareSpaceResult:
          if(SpatialEntityShareSpace != null)
          {
            int sizeofUInt64 = 8;
            int sizeofInt32 = 4;
            int sizeofPrincipalType = Marshal.SizeOf(Enum.GetUnderlyingType(typeof(OVRPlugin.PrincipalType)));
            int sizeofCharPtr = 4; // This is assuming the OS is 32-bit, this would need to be 8 if it's a 64-bit OS
            int offset = 0;

						// TODO: Use custom bitpacker in favor of BitConverter for perf
            UInt64 requestId = BitConverter.ToUInt64(eventDataBuffer.EventData, offset);
            offset += sizeofUInt64;
            int result = BitConverter.ToInt32(eventDataBuffer.EventData, offset);
            offset += sizeofInt32;
            UInt32 numSpaces = BitConverter.ToUInt32(eventDataBuffer.EventData, offset);
            offset += sizeofInt32;

            OVRPlugin.SpatialEntityUuid[] uuids = new OVRPlugin.SpatialEntityUuid[OVRPlugin.SpatialEntitySharingMaxSpacesPerRequest];
            for (int i = 0; i < OVRPlugin.SpatialEntitySharingMaxSpacesPerRequest; i++) {
              OVRPlugin.SpatialEntityUuid uuid;
              uuid.Value_0 = BitConverter.ToUInt64(eventDataBuffer.EventData, offset);
              offset += sizeofUInt64;
              uuid.Value_1 = BitConverter.ToUInt64(eventDataBuffer.EventData, offset);
              offset += sizeofUInt64;
              uuids[i] = uuid;
            }
            UInt64[] spaces = new UInt64[OVRPlugin.SpatialEntitySharingMaxSpacesPerRequest];
            for (int i = 0; i < OVRPlugin.SpatialEntitySharingMaxSpacesPerRequest; i++) {
              spaces[i] = BitConverter.ToUInt64(eventDataBuffer.EventData, offset);
              offset += sizeofUInt64;
            }

            UInt32 numPrincipals = BitConverter.ToUInt32(eventDataBuffer.EventData, offset);
            offset += sizeofInt32;
						// TODO: Update the way User Ids are referenced, remove the usage of 'Principals'
            OVRPlugin.Principal[] principals = new OVRPlugin.Principal[numPrincipals];
            for (int i = 0; i < numPrincipals; i++) {
              OVRPlugin.Principal principal;
              principal.IdType = (OVRPlugin.PrincipalType)BitConverter.ToUInt32(eventDataBuffer.EventData, offset);
              offset += sizeofPrincipalType;
              principal.Id = System.Text.Encoding.ASCII.GetBytes("Not fully implemented");
              offset += sizeofCharPtr;
              principals[i] = principal;
            }
            SpatialEntityShareSpace(requestId, result >= 0, spaces, uuids, numSpaces, principals, numPrincipals);
          }
          break;
#endif // OVR_INTERNAL_CODE
#if OVR_INTERNAL_CODE
