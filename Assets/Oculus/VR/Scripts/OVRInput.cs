...
public static class OVRInput
{
	...
	private static float GetResolvedAxis1D(Axis1D virtualMask, RawAxis1D rawMask, Controller controllerMask)
	{
		...
		for (int i = 0; i < controllers.Count; i++)
		{
			...
			if (ShouldResolveController(controller.controllerType, controllerMask))
			{
				...
				if ((RawAxis1D.LTouchpadForce & resolvedMask) != 0)
				{
					float axis = controller.currentState.LTouchpadForce;
					maxAxis = CalculateAbsMax(maxAxis, axis);
				}
				if ((RawAxis1D.RTouchpadForce & resolvedMask) != 0)
				{
					float axis = controller.currentState.RTouchpadForce;
					maxAxis = CalculateAbsMax(maxAxis, axis);
				}
	...
	private abstract class OVRControllerBase
	{
		...
		public class VirtualAxis1DMap
		{
			...
			public RawAxis1D PrimaryTouchpadForce       = RawAxis1D.None;
			...
			public RawAxis1D SecondaryTouchpadForce     = RawAxis1D.None;
			...
			public RawAxis1D ToRawMask(Axis1D virtualMask)
			{
				...
				if ((virtualMask & Axis1D.PrimaryTouchpadForce) != 0)
					rawMask |= PrimaryTouchpadForce;
				...
				if ((virtualMask & Axis1D.SecondaryTouchpadForce) != 0)
					rawMask |= SecondaryTouchpadForce;
	...
	private class OVRControllerTouch : OVRControllerBase
	{
		...
		public override void ConfigureAxis1DMap()
		{
			...
			axis1DMap.PrimaryTouchpadForce       = RawAxis1D.LTouchpadForce;
			...
			axis1DMap.SecondaryTouchpadForce     = RawAxis1D.RTouchpadForce;
	...
	private class OVRControllerLTouch : OVRControllerBase
	{
		...
		public override void ConfigureAxis1DMap()
		{
			...
			axis1DMap.SecondaryTouchpadForce     = RawAxis1D.None;
	...
	private class OVRControllerRTouch : OVRControllerBase
	{
		...
		public override void ConfigureAxis1DMap()
		{
			...
			axis1DMap.PrimaryTouchpadForce       = RawAxis1D.RTouchpadForce;
	...
	private class OVRControllerHands : OVRControllerBase
	{
		...
		public override void ConfigureAxis1DMap()
		{
			...
			axis1DMap.PrimaryTouchpadForce       = RawAxis1D.None;
...
#if OVR_INTERNAL_CODE
		public override void SetControllerLocalizedVibration(HapticsLocation hapticsLocationMask, float frequency, float amplitude)
		{
			if ((hapticsLocationMask & HapticsLocation.Hand) != 0)
			{
