...
Shader "Sanctuary/OzDimension"
{
		Properties
		{
			...
			_OzPosition("Oz Position", Vector) = (0,0,0,0)
			_OzRippleStrength("Oz Ripple Strength", Range(0 , 1)) = 1
			...
			SubShader
		{
			...
			Pass
			{
				...
				uniform float3 _OzPosition;
				uniform half _OzRippleStrength;
				...
				fixed4 frag(vertexOutput i) : SV_Target
				{
					...
					//distance gradient
					half distanceToOz = pow(distance(_OzPosition, i.worldPos), 1.5);
					half distanceToOzMask = saturate(1 - (distanceToOz * 0.2));
					half distanceRipple = saturate(sin((distanceToOz * 6) + (_Time.w * 2) ) * 0.5 + 0.25) * distanceToOzMask * 4;
					// noisy mask
					...
					half4 litTexture = (finalLighting * _Color) + (mainTextureTriPlanar.rrrr   * ((noisyMask * 0.85) + (distanceRipple * _OzRippleStrength)));
