...
#include "UnityShaderVariables.cginc"
            ...
            fixed4 frag(interpolators i) : SV_Target {
                ...
                half noisyMask = ((((_Time.y * 0.17 + (mainTexture.b * 1.73)) + (_Time.y * 0.31 + (
                    mainTexture.g * 1.52))) % 1.0));
                noisyMask = abs(noisyMask - 0.5) * (_Color.r * 6);
                // recentering it around 0.5, scaling it by color
                half animatedStarTextures = mainTexture.r * ((noisyMask * 0.85) + (distanceRipple *
                    1));
