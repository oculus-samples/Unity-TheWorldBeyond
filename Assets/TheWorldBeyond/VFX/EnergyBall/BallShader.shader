...
Shader "TheWorldBeyond/BallShader"
{
    ...
    half4 GetWobbleAmount(float3 worldPos, float time) {
        half4 wobbleVector = float4(
                sin((worldPos.x * 3.2) + (time * 6.3)),
                sin((worldPos.y * 5.0) + (time * 5.13)),
                sin((worldPos.z * 2.0) + (time * 3.23)),
                0.0);
        return wobbleVector;
    }
    ENDCG
...
#include "UnityShaderVariables.cginc"
            ...
            fixed4 frag(v2f i) : SV_Target {
                ...
                half coreColorThrobControl = (sin((_Time.y * 5.0) + (i.worldPos.z * 4)) + sin(
                        (_Time.y * 7.1) + (unity_WorldTransformParams.x * 2))) * 2 + 5;
