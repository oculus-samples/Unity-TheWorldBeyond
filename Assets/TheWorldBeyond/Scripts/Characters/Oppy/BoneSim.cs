...
namespace TheWorldBeyond.Character.Oppy
{
    ...
    public class BoneSim : MonoBehaviour
    {
        ...
        public class Bone
        {
            ...
            [Tooltip("The transform that the RootBone is aiming toward. If no transform specified an offset Position will be assigned")]
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            for (var i = 0; i < Chain.Length; i++)
            {
                ...
                Gizmos.DrawSphere(Chain[i].RootBone.position, m_gizmoSize * 0.5f);
                ...
                Gizmos.DrawLine(Chain[i].RootBone.position, Space.TransformPoint(Chain[i].SimPos));
            }
        }
        ...
        public void Init()
        {
            ...
            for (var i = 0; i < Chain.Length; i++)
            {
                ...
                {
                    Chain[i].StartPose = new Pose(Chain[i].RootBone.localPosition, Chain[i].RootBone.localRotation);
            ...
            for (var i = 0; i < Chain.Length; i++)
            {
                ...
                Chain[i].ForVec = Chain[i].Child != null ?
                    Chain[i].RootBone.InverseTransformVector(Chain[i].Child.position - Chain[i].RootBone.position).normalized :
                    Chain[i].RootBone.InverseTransformVector(Chain[i].RootBone.position - Chain[i].RootBone.parent.position).normalized;
                ...
                Chain[i].TargetPos = Chain[i].Child != null ?
                    ...
                    Space.InverseTransformPoint(Chain[i].RootBone.TransformPoint(Chain[i].ForVec));
        ...
        public void Tick()
        {
            ...
            for (var i = 0; i < Chain.Length; i++)
            {
                ...
                if (ForceUpdate)
                {
                    Chain[i].RootBone.localPosition = Chain[i].StartPose.position;
                    Chain[i].RootBone.localRotation = Chain[i].StartPose.rotation;
                }
                ...
                // Animated upvector
                m_animatedUpVec = Chain[i].RootBone.rotation * Chain[i].UpVec;
                ...
                // pose RootBone
                Chain[i].RootBone.rotation = Quaternion.Lerp(Chain[i].RootBone.rotation,
                    Quaternion.LookRotation(Space.TransformPoint(Chain[i].SimPos) - Chain[i].RootBone.position, m_animatedUpVec) * Chain[i].LocalRotationAxis,
                    Blend);
            }
        }
        ...
        {
            var max = Mathf.Max(direction.x, direction.y, direction.z);
            if (max == direction.x)
                return Vector3.up;
