...
#include "AutoLight.cginc"
            ...
            fixed4 frag(vertexOutput i) : SV_Target {
                ...
                half4 summedTexture = mainTexture + ((detailTexture - 0.5) * (_DetailTexStrength *
                    mainTexture.a));
...
#include "AutoLight.cginc"
            ...
            fixed4 frag(vertexOutput i) : SV_Target {
                ...
                //fogging
                float FoggingRange = clamp(
                        ((distance(_WorldSpaceCameraPos, i.worldPos) - _FogStartDistance) / (
                            _FogEndDistance - _FogStartDistance)),
                        0.0,
                        1.0);
                FoggingRange = fastPow(FoggingRange, _FogExponent);
                half4 foggedColor = lerp(
                        finalLighting,
                        half4(0, 0, 0, 0),
                        (FoggingRange * _FogStrength));

                half4 finalColor = foggedColor * coloredTexture;
                //fog is black, so light strength fades to 0 with depth
