...
#include "AutoLight.cginc"
            ...
            fixed4 frag(vertexOutput i) : SV_Target {
                ...
                half distanceRipple = saturate(
                                sin((distanceToOppy * 6) + (_Time.w * 2)) * 0.5 + 0.25) *
                        distanceToOppyMask
                        * 4;
                // noisy mask
                half noisyMask = ((((_Time.y * 0.06 + (mainTextureTriPlanar.b * 1.73)) + (_Time.y *
                    0.19 + (mainTextureTriPlanar.g * 1.52))) % 1.0));
                ...
                half4 finalLighting = (half4(_LightColor0.rgb, 0.0) * lightingRamp) + (half4(
                        ...
                        0));
                half4 litTexture = (finalLighting * _Color) + (mainTextureTriPlanar.rrrr * ((
                    noisyMask * 0.85) + (distanceRipple * _OppyRippleStrength)));
                ...
                half maskRipple = saturate(sin((distanceToBall * 20) + (_Time.w * 2)) * 0.5 + 0.25)
                        * saturate(1 - (distanceToBall * 0.5)) * 0.7;
